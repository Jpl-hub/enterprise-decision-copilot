...
class CompareCompaniesTool:
    ...
    def run(self, context: WorkflowContext, analytics_service: AnalyticsService) -> ToolResult:
        comparison = analytics_service.compare_companies(
            [str(match["company_code"]) for match in context.matches]
        )
...
class IndustryTrendTool:
    ...
    def run(self, context: WorkflowContext, analytics_service: AnalyticsService) -> ToolResult:
        ...
        macro_digest = analytics_service.get_macro_digest()
        top_industries = "、".join(
            item["industry_name"] for item in overview.get("top_industries", [])[:4]
        ) or "暂无"
        macro_line = "；".join(
