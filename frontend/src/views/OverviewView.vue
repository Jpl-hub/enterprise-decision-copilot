...
const aiPillarHighlights = computed(() => (aiStackSummary.value?.pillars || []).slice(0, 3).map((item) => ({ name: item.name, score: `${Math.round((item.readiness_score || 0) * 100)}`, summary: item.summary })));
...
function getRiskFlagsText(flags?: unknown) { return Array.isArray(flags) && flags.length ? flags.slice(0, 2).join('；') : '当前未触发显著风险标记，建议继续跟踪经营与行业变量。'; }
